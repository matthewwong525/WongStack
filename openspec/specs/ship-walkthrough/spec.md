# ship-walkthrough Specification

## Purpose

The opt-in staging walkthrough: the gate `/ship` runs between green CI and the merge in repos that adopted it. Covers how consent is detected from repo state, how a change's own OpenSpec scenarios become browser journeys, what the walk captures, how it is graded against the scenario's written `THEN`, the five verdicts and what each does to the merge, failure recovery and reseeding, and where the evidence lands.
