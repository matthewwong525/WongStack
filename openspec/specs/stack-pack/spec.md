...
The opt-in Cloudflare stack pack in the WongStack payload: three zero-config D1 pipeline scripts, a seed template, guided config fragments, and the `wiki/stack/` pipeline docs — installed and refreshed only for a repo that opted in (`components.stackPack: true`), leaving WongStack byte-for-byte stack-agnostic for every repo that declines. The scripts auto-apply migrations on deploy (production on the default branch, staging elsewhere), swap the binding for previews, and rebuild staging from a checked-in seed without ever touching production.
