...
The pack's CI runs the application's own test suite as an ordinary check, in parallel with the build
and deploy job, and the change loop grows that suite as a standing task convention. Distinct from the
