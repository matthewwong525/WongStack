# improve-plan-output Specification

## Purpose
Where and in what format `/improve` writes its plans — OpenSpec change folders when the target repo plans with OpenSpec, `plans/` files otherwise — and how the plan-consuming variants locate and track those plans in each mode.
