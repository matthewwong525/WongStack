...
- [ ] 4.1 `/save` the change and record the first CI run: Test job time, Stryker time, and whether the cache restored a file (expected: no file on the first run, unless `main` has one with the same hash).
