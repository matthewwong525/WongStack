...
- [ ] 1.1 Create `wiki/recommended-stack.md` per the progressive-disclosure rulebook (`wiki/wiki-style.md` + `wiki/voice.md`): a topic title, a strong opener that states up front it's a recommendation and not a requirement
...
- [ ] 2.1 Link the doc from `wiki/README.md` as an optional appendix, visually separate from the core "Where to find things" process list
- [ ] 2.2 Add cross-links: from `wiki/development/the-change-loop.md` (sideways to the tips) and, if `evolve-change-loop` landed, to the secrets-convention page
