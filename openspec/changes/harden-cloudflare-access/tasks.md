...
- [ ] 7.1 Stand Access up on a custom domain per the rewritten runbook and confirm all three callers behave: anonymous challenged, service token admitted, logged-in browser served
- [ ] 7.2 Confirm `/walk` reaches a gated preview
- [ ] 7.3 Confirm the scoped patterns gate the app's hostnames and leave an unrelated Worker on the same subdomain open
- [ ] 7.4 Run `/wong-cloudflare` end to end and confirm the smoke test reports correctly on both a public and a gated app
