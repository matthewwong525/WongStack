...
- [x] 5.1 Bump `VERSION` to 18.2.0 and add a newest-first `CHANGELOG.md` entry. It covers the example move and says that an installed repo with a root `.dev.vars` moves it to `app/.dev.vars` by hand. It also covers the worktree-secrets helper, the `/ship` promote, and the rule.
