...
- **2026-09-26** — ship distill: no repeatable fact. The two session facts (the wongstack-cloud sync thread and the downstream-plan check) are change-specific; the cache design lives in `test.yml`'s header comment and the CHANGELOG.
- **2026-09-26** — archived to `openspec/changes/archive/2026-09-26-incremental-mutation-testing/` for shipping; specs were already synced, so the archive used `--skip-specs` after an equality check.
