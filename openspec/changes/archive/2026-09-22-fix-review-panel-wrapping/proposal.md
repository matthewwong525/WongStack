...
- **2026-09-22** — Merge checkpoint: `main` moved to 16.6.0 (shorter review page planning) while this change waited. Resolved the two conflicts as the union of intent — the changelog keeps both entries and this release renumbers from 16.5.1 to **16.6.1**. The kit fix itself did not conflict; 16.6.0 touched `.panel ol.changes li .kind`, not `.panel code`.
