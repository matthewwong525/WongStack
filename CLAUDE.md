...
This repo is **WongFramework** — a stack-agnostic Claude Code workflow framework, distributed as a **template you clone and work from**. The whole payload is the repo root: [`.claude/skills/`](.claude/skills/), [`docs/`](docs/), [`VERSION`](VERSION), [`CHANGELOG.md`](CHANGELOG.md), and the `WONG-FRAMEWORK` block in this file. The [`install-wong-framework`](.claude/skills/install-wong-framework/SKILL.md) skill copies that set (minus itself) into *other* repos. See the [README](README.md) for the user story.

It's a **meta-repo** that ships the framework *and* dogfoods it — the block below applies here too. Don't run `/install-wong-framework` here; it's the source, not a target.
...
<!-- WONG-FRAMEWORK:BEGIN — generic framework conventions. The installer lifts this block verbatim into a target repo's CLAUDE.md, so keep it free of repo-specifics. Edit freely between the markers. -->
...
- **GitHub Actions is the build gate.** Don't run a build/test locally as a prerequisite —
  push and let CI run. The framework skills wait for the checks and fix failures.
- **Use the framework skills** instead of hand-rolling the workflow:
