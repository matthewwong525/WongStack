...
6. **Workflow fit** — confirm the loop suits them: plan in `openspec/` (`/plan`), build (`/continue`), checkpoint behind GitHub-Actions-as-only-gate (`/save`), merge + archive (`/ship`). Thin/absent CI is fine — just nothing to wait for.
