...
> 5. **`.claude/skills/`** — existing skills, especially collisions: `save`, `preview`, `continue`, `ship`, `document`, `improve`.
...
{ "version": "$LATEST", "installedAt": "<existing, or today>", "updatedAt": "$(date +%F)",
  "components": { "skills": ["save","preview","continue","ship","document","improve"], "claudeMd": true, "docs": true } }
EOF
```
