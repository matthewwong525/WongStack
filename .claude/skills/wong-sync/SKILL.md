...
The WongStack round trip in one pass. `/install-wong-stack` installs once; from then on this skill keeps the install current **and** keeps WongStack current with you:
...
```
- **No manifest** → WongStack isn't installed here; stop and point at `/install-wong-stack`.
- **This repo IS a WongStack source** (`$ROOT/VERSION` exists alongside `$ROOT/.claude/skills/install-wong-stack/`) → **stop** — the source has nothing to sync with itself.
...
```
Show what's new since the installed version: the `$WS/CHANGELOG.md` entries newer than the manifest's `version`.
...
**No `$BASE`** (an install that predates this skill): say so up front, fall back to a plain two-way walk of local vs `$WS` for this one sync (show each diff, ask keep / take-upstream / candidate), and record `$WS_HEAD` at Step 6 so every later sync is three-way.

## Step 3 — pull leg (upstream → here, no git)

Apply the **upstream updates** to `$ROOT`'s working tree: list them with a one-line summary each and offer batch approval (per-file inspection on request). Walk **true conflicts** one at a time with a three-way view — keep local / take upstream / merge by hand. For `CLAUDE.md`, replace only the marker block; everything outside it is this repo's own.
...
Older manifests just gain the new keys here — nothing breaks on a v1 manifest. Set `commit` to `$WS_HEAD` (pre-contribution HEAD of the default branch); keep `components` matching reality. If the target still carries a `contribute-wong-stack` skill or symlink, offer to remove it — `/wong-sync` supersedes it.

## Step 7 — report

Pulled (files + one-liners, and that they await `/save`); conflicts and how each resolved; contributed (files + rationales + the PR URL, or the parked branch name on the degraded path); skipped candidates; new manifest `version`/`commit`; fork recorded, if one was created.
