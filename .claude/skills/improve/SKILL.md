---
name: improve
description: Survey any codebase as a senior advisor and produce prioritized, self-contained implementation plans for OTHER models/agents to execute — written as OpenSpec change folders when the target repo plans with OpenSpec (has openspec/changes/), as plans/NNN-*.md files otherwise. Strictly read-only on source code — never implements, fixes, or refactors anything itself. Use when asked to audit a codebase, find improvement opportunities (bugs, security, performance, test coverage, tech debt, migrations, DX), suggest features or where to take the project next (roadmap, product direction), or generate handoff plans for another agent to implement. Includes a `docs` variant — `/improve docs` — that specializes the audit for a docs/ progressive-disclosure wiki (broken links/anchors, orphans, weak openers, duplication, stale docs) per references/docs-audit-playbook.md. Adapted from shadcn/improve (MIT).
user-invocable: true
license: MIT
metadata:
  author: shadcn
...
- `docs` → specialize the audit for a **`docs/` progressive-disclosure wiki**. Run Recon (read `docs/wiki-style.md` — the rulebook), then audit the wiki against [references/docs-audit-playbook.md](references/docs-audit-playbook.md) — a docs-specific superset of the *docs* category (structural integrity: broken links/anchors, orphans, hub gaps; openers & titles; one-topic-one-page; navigation; staleness; coverage) — then plan. Composes with `quick`/`deep`/`--issues`. Docs plans are applied by a human — via `/continue` → `/save` → `/ship` in OpenSpec mode, `/save` → `/ship` otherwise (the playbook's "Planning & applying docs fixes") — never `execute`.
