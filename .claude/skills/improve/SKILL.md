---
name: improve
description: Survey any codebase as a senior advisor and produce prioritized, self-contained implementation plans for OTHER models/agents to execute — written as OpenSpec change folders when the target repo plans with OpenSpec (has openspec/changes/), as plans/NNN-*.md files otherwise. Strictly read-only on source code — never implements, fixes, or refactors anything itself. Use when asked to audit a codebase, find improvement opportunities (bugs, security, performance, test coverage, tech debt, migrations, DX), suggest features or where to take the project next (roadmap, product direction), or generate handoff plans for another agent to implement. Includes a `docs` variant — `/improve docs` — that specializes the audit for the repo's progressive-disclosure wiki (root `wiki/`, falling back to `docs/`) — broken links/anchors, orphans, weak openers, duplication, stale docs — per references/docs-audit-playbook.md. Adapted from shadcn/improve (MIT).
user-invocable: true
license: MIT
metadata:
  author: shadcn
  adapted-from: shadcn/improve
---
...
> **Adapted from [shadcn/improve](https://github.com/shadcn/improve)** (MIT — see [LICENSE.md](LICENSE.md)). The audit → vet → plan workflow, the [audit playbook](references/audit-playbook.md), the [plan template](references/plan-template.md), and the finding format are shadcn's, carried pretty much verbatim. WongStack adds two things: the **`docs` variant** (see Invocation variants) with its [docs audit playbook](references/docs-audit-playbook.md), and **OpenSpec mode** ([openspec-plans.md](references/openspec-plans.md)) — in a repo that plans with OpenSpec, plans are written as change folders instead of `plans/` files.

## Hard Rules

1. **Never modify source code yourself.** No edits, no fixes, no "quick wins while you're in there." The ONLY files you may create or modify live under `plans/` in the repo root — or under `advisor-plans/` when `plans/` already exists for an unrelated purpose (create the chosen directory if absent) — or, when the repo plans with OpenSpec (it has `openspec/changes/`), under `openspec/changes/` per [references/openspec-plans.md](references/openspec-plans.md). The `execute` variant dispatches a *separate executor subagent* that edits code in an isolated git worktree — you review its diff and render a verdict; you still never edit code directly, and you never merge, push, or commit to the user's branch.
...
**OpenSpec mode.** If the repo plans with OpenSpec (it has `openspec/changes/` at the root), plans do NOT go in `plans/` — each selected finding becomes an OpenSpec change folder (`openspec/changes/<slug>/` with proposal, tasks, design and delta specs as needed), so the repo's own loop can implement it. Read [references/openspec-plans.md](references/openspec-plans.md) before writing the first change: it maps every plan-template section onto the OpenSpec artifacts (the quality bar below applies unchanged), and replaces the `plans/README.md` reconciliation with a check of active + archived changes. The rest of this phase reads the same in both modes.
...
- `docs` → specialize the audit for the repo's **progressive-disclosure wiki**. Resolve the wiki root first: `wiki/` if it exists, else `docs/`. Run Recon (read `wiki-style.md` at the wiki root — the rulebook), then audit the wiki against [references/docs-audit-playbook.md](references/docs-audit-playbook.md) — a docs-specific superset of the *docs* category (structural integrity: broken links/anchors, orphans, hub gaps; openers & titles; one-topic-one-page; navigation; staleness; coverage) — then plan. Composes with `quick`/`deep`/`--issues`. Docs plans are applied by a human — via `/continue` → `/save` → `/ship` in OpenSpec mode, `/save` → `/ship` otherwise (the playbook's "Planning & applying docs fixes") — never `execute`.
...
- `review-plan <file>` → critique an existing plan in `plans/` (or, in OpenSpec mode, a change folder by slug) against the template's standards and tighten it. If you authored the plan in this same session, also have a fresh-context subagent read it cold and report ambiguities — self-critique misses gaps you mentally fill from context the executor won't have.
- `execute <plan>` → dispatch a cheaper executor subagent on one plan (isolated worktree), then review its diff like a tech lead — re-run done criteria, check scope, read the code — and render a verdict. Treat the executor's diff as untrusted until reviewed: verify every hunk traces to a plan step and reject any out-of-scope change, however plausible it looks. Requires a host agent that can spawn subagents in an isolated worktree; if yours can't, say so and hand the plan over for manual execution instead. In OpenSpec mode, prefer the repo's own loop (`/continue <slug>`) and see the `execute` adaptations in [references/openspec-plans.md](references/openspec-plans.md). **Read [references/closing-the-loop.md](references/closing-the-loop.md) before the first dispatch.**
- `reconcile` → process what happened since last session: verify DONE plans, investigate BLOCKED ones, refresh drifted TODOs, retire dead findings. See [references/closing-the-loop.md](references/closing-the-loop.md); in OpenSpec mode, reconcile against `openspec list` + the archive per [references/openspec-plans.md](references/openspec-plans.md).
- `--issues` (modifier on any planning invocation) → also publish each written plan as a GitHub issue via `gh`, URL recorded in the plan and index (in OpenSpec mode: proposal + tasks as the body, URL in the proposal). Only with the explicit flag. **Before creating any issue, check whether the repo is public (`gh repo view --json visibility`). If it is, warn the user that issues are publicly visible and get explicit confirmation before publishing any plan that describes a security vulnerability, credential location, or other sensitive finding.** See [references/closing-the-loop.md](references/closing-the-loop.md).
