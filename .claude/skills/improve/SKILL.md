---
name: improve
description: Survey any codebase as a senior advisor and produce prioritized, self-contained implementation plans for OTHER models/agents to execute. Strictly read-only on source code — never implements, fixes, or refactors anything itself. Use when asked to audit a codebase, find improvement opportunities (bugs, security, performance, test coverage, tech debt, migrations, DX), suggest features or where to take the project next (roadmap, product direction), or generate handoff plans for another agent to implement. Includes a `docs` variant — `/improve docs` — that specializes the audit for a docs/ progressive-disclosure wiki (broken links/anchors, orphans, weak openers, duplication, stale docs) per references/docs-audit-playbook.md. Adapted from shadcn/improve (MIT).
user-invocable: true
license: MIT
metadata:
  author: shadcn
...
> **Adapted from [shadcn/improve](https://github.com/shadcn/improve)** (MIT — see [LICENSE.md](LICENSE.md)). The audit → vet → plan workflow, the [audit playbook](references/audit-playbook.md), the [plan template](references/plan-template.md), and the finding format are shadcn's, carried pretty much verbatim. WongStack adds one thing: the **`docs` variant** (see Invocation variants) and its [docs audit playbook](references/docs-audit-playbook.md).
