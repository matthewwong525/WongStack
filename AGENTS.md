...
- **Stay in sync with WongStack** — `/wong-sync` reads upstream's own latest instructions before it
  decides anything (so the run uses upstream's logic, not the installed copy, without rewriting
  itself to do it), then *analyses instead of overwriting*: it reads what upstream lets you do
  against what this repo already does, asks only what the repo itself cannot answer, and proposes
  the worthwhile gap — when in doubt, it proposes. **The run changes no file.** Copies, updates and
  grafts all become tasks in **one** OpenSpec change whose proposal is an *after-picture*: how this
  repo works once it lands, what you gain, and what you lose. Nothing happens until you review it
  and `/apply` — the only exception is a fresh install, where the copy is the install. It **never
  modifies a file with local authorship** — one edited byte defeats the proof — so local
  customization is safe and a capability you already solve your own
