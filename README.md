...
- **Knowledge has two homes.** Reusable *processes* → the `docs/` progressive-disclosure tree (via `/dream`). A change's *specifics* → its proposal and specs, preserved in `openspec/changes/archive/` when it ships. The archive is the project's searchable record of what shipped.
...
```
Installing into a target repo gives it: a merged `CLAUDE.md`, the `openspec/` scaffold + `/opsx:*` commands (via `openspec init`), `.claude/skills/{explore,plan,apply,save,continue,ship,dream,improve}/`, `docs/README.md` + `docs/wiki-style.md`, a `.claude/.wong-stack.json` manifest, and — if you opt in — the `auto-push` Stop hook.
