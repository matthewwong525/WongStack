...
| Memory key (`CLOUDFLARE_MEMORY_TOKEN`) | The git-ignored `.env` | Not a Cloudflare token. It opens one repo's memory store through that repo's production Worker, and nothing else. An admin key reads every transcript in that store; a member key reads only its own. It is never a GitHub secret. |
