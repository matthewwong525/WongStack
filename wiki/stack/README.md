...
- [D1 pipeline](d1-pipeline.md) — *how* data ships: the two-database model (one binding, prod + staging), migrations that auto-apply on deploy, timestamp-prefixed migrations and the additive/order-independent rule, the seeded-staging model and `db:reset:staging`, and the three prod-recovery runbooks — Time Travel, never hand-apply schema, and reconciling `d1_migrations` when prod drifts.
