...
### 2. Access service token — only if you have a login wall, and `/walk` can mint it

If you adopted [Cloudflare Access](cloudflare-access.md), your preview URLs sit behind a login and a headless browser gets challenged. The two values live in the primary worktree's durable `.env` per the [secrets convention](../development/secrets.md); `/walk` resolves that file even when invoked from a linked worktree:
...
**You don't have to put them there yourself.** The first walk that meets the login wall mints the token, stores the pair, and retries — see [when the walk can't get in](#when-the-walk-cant-get-in). This rung is what that repair produces, documented so you know what appeared in your `.env` and why. Setting the values by hand still works and skips the mint.
...
```
  ...
  /save ───── push, wait for CI, resolve the per-commit preview URL
     ...
     │             blocked by Access or a refused token? heal once, retry once
     │
...
Two blocks stop a walk before it sees the app, and `/walk` holds the credential that fixes both — so it fixes them rather than sending you an errand:

| Block | What `/walk` does | If the retry is blocked again |
|---|---|---|
| **Browser Run refuses the token** — it was never widened into Browser Rendering Edit | runs [the widen protocol](../../.claude/skills/wong-cloudflare/references/permission-groups.md) on the token itself, then retries once | `UNKNOWN`, naming the widen that didn't take |
| **The preview answers with an Access login wall** and no service token is stored | mints a service token named for the repo, confirms the [Access](cloudflare-access.md) policy accepts it, writes the pair to the primary worktree's durable `.env`, then retries once | `UNKNOWN`, naming the mint that didn't help |

Both repairs are already authorized: pasting a two-permission token *is* [the authorization to widen it](cloudflare-credentials.md#the-widen-is-pre-authorized), which is the same standing permission `/wong-cloudflare` runs on. The walk reports what it granted or minted, and never prints a credential value.

**One heal and one retry per block** — never a loop. A block that survives its repair is `UNKNOWN` with the attempt named, so an unverified walk never looks like an untried one.

## When a walk fails

The evidence is posted first — a failing walk's screenshots are the whole point — then staging is reset (`npm run db:reset:staging`).
...
- **Not a gate on `/ship` — though `/ship` does walk.** The walk was once a *merge gate* between green CI and the merge, and that is what was removed and stays removed. Making it a gate forced everything around it: a walk that couldn't run had to block, retries had to share `/ship`'s attempt budget, and the only moment you could see your app was the moment you were done with it. `/ship` now runs one walk as an **evidence step** instead: `NONE`, `UNKNOWN`, and `TIMEOUT` are reported and the merge proceeds on CI, so an unrunnable walk still blocks nothing — the sharpest of the three pains. A `FAILURE` stops and asks you to fix or merge anyway, which is a decision handed to a human rather than a condition evaluated by a skill. And because the walk is invocable any time, the moment you see your app is still whenever you choose.
...
- **Not an unbounded fix loop.** `/walk` fixes a failure in *this change's own code*, twice at most, and repairs each access block once. Those bounds are the decision, not an implementation detail: an agent that keeps fixing and re-walking until something passes will eventually pass something, and the walk's value comes entirely from being willing to report a failure. Out-of-scope failures aren't touched at all — that's how a walk stays this change's acceptance rather than becoming a licence to edit the repo.
