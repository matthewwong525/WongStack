...
| **D1** | The database | Cloudflare's SQLite. Migrations in `schema/migrations/` apply automatically on deploy — see the [D1 pipeline](d1-pipeline.md). |
...
**Merge = deploy.** A PR's merge to the default branch *is* the production release: Workers Builds runs the [build wrapper](d1-pipeline.md#the-build-wrapper), applies pending migrations to prod, and ships the new bundle. No separate deploy step to forget, no drift between "merged" and "live."

**One runtime, front to back.** The SPA, the APIs, and the data binding all live in a single Worker with one config file (`wrangler.jsonc`). An agent reasoning about a change sees the whole surface in one place instead of stitching a frontend host to a separate backend to a separate database.

**Cheap previews are the inner loop.** Every branch push gets its own preview URL bound to a [staging database](d1-pipeline.md#the-two-database-model) — a real, shareable deploy to exercise a change against, with no local build gate. `/save` pushes and hands back the URL; that URL, not a localhost server, is where you confirm the change works.

## Next

- How data ships and how a bad migration is recovered: the [D1 pipeline](d1-pipeline.md).
