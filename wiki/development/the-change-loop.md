...
- **[`/ship`](../../.claude/skills/ship/SKILL.md)** — verify the feature/default branches, archive the change to `openspec/changes/archive/YYYY-MM-DD-<name>/` through `openspec-archive-change`, invoke `/save` once in shipping context so that exact archive commit is pushed and gated, then squash-merge and delete the remote branch worktree-safely. It owns no duplicate commit, PR, or branch-CI implementation.
...
An **unverifiable** gate is not an absent one. When the check state can't be read, an ordinary
`/save` reports it as unverified and carries on — it's a checkpoint — while `/save` in shipping
context returns an unmergeable result and `/ship` stops rather than reinterpret or repeat it.
