...
- **[`/explore`](../../.claude/skills/explore/SKILL.md)** — think a problem through with related question groups, and own the single [question round](#asking-before-drafting) at the transition to planning. It **always runs**: invoke it yourself for as long as the thinking needs, or `/plan` invokes it for you in a bounded pass that reads the conversation, investigates only the gap, asks once, and returns. Fronts the `openspec-explore` skill. Nothing is written yet.
