# Working on WongStack

Working on WongStack means editing the toolkit itself — this repo is the meta-repo that *ships* WongStack and *dogfoods* it at once. This section covers how to change what downstream repos receive without breaking their next install or update.

The **payload** is the set that [`/wong-sync`](../../.claude/skills/wong-sync/SKILL.md) pulls into other repos and keeps current — a fresh install (fronted by [`/wong-setup`](../../.claude/skills/wong-setup/SKILL.md)) is the same manifest-driven sync against an empty base: the workflow skills under [`.claude/skills/`](../../.claude/skills/), the [OpenSpec](https://github.com/Fission-AI/OpenSpec) planning layer (`openspec/` plus the generated `opsx` commands and `openspec-*` skills), this [`wiki/`](../README.md) wiki, [`VERSION`](../../VERSION), [`CHANGELOG.md`](../../CHANGELOG.md), and the `WONG-STACK:BEGIN/END` block in [`CLAUDE.md`](../../CLAUDE.md). The canonical file-by-file list lives in one place: the [payload manifest](../../.claude/skills/wong-sync/references/payload-manifest.md) inside `wong-sync`. Everything else in the repo is scaffolding around it.

**Editing the payload is a release.** Any change a downstream repo would receive has to be versioned and explained, or the installer's updater can't detect it — so a payload edit always ends by bumping [`VERSION`](../../VERSION) (semver) and adding a newest-first [`CHANGELOG.md`](../../CHANGELOG.md) entry in the same change. The "Working on WongStack" notes in [`CLAUDE.md`](../../CLAUDE.md) are the short form of this rule; the pages here are the long form.

## Processes

- [The change loop](the-change-loop.md) — how work moves from idea to shipped, archived spec: `/explore → /plan → /apply → /save → /continue → /ship`, each a thin verb over an OpenSpec step, with the change as a living handoff (Status header + append-only Decision log + PR-body mirror).
- [Adding a skill](adding-a-skill.md) — create a new workflow skill and wire it through every surface that installs, versions, and advertises the payload.
- [Required tools](required-tools.md) — the whole toolchain is `git`, `gh`, and `openspec`: why it stays that small, and how the payload handles JSON without a standalone `jq`.
- [Secrets and environment variables](secrets.md) — the `.env.example`-as-source-of-truth convention: real secrets stay out of git, the committed template lists every variable, contributors bootstrap a local file from it.
- [Contributing upstream](../contributing.md) — the other side of the payload: how a target repo sends an improvement back with `/wong-sync contribute`, and the generality bar it has to clear before you'd merge it here.
