...
The **payload** is the set that [`/wong-sync`](../../.claude/skills/wong-sync/SKILL.md) brings into other repos and keeps current — a fresh install (fronted by [`/wong-setup`](../../.claude/skills/wong-setup/SKILL.md)) is the same manifest-driven sync in the case where every payload file happens to be absent: the workflow skills under [`.claude/skills/`](../../.claude/skills/), the [OpenSpec](https://github.com/Fission-AI/OpenSpec) planning layer (`openspec/` plus the `openspec-*` skills `openspec init` generates), this [`wiki/`](../README.md) wiki, [`VERSION`](../../VERSION), [`CHANGELOG.md`](../../CHANGELOG.md), and the `WONG-STACK:BEGIN/END` block in [`CLAUDE.md`](../../CLAUDE.md). The canonical file-by-file list lives in one place: the [payload manifest](../../.claude/skills/wong-sync/references/payload-manifest.md) inside `wong-sync`. Everything else in the repo is scaffolding around it.

**Editing the payload is a release.** Any change a downstream repo would receive has to be versioned and explained, or the installer's updater can't detect it — so a payload edit always ends by bumping [`VERSION`](../../VERSION) (semver) and adding a newest-first [`CHANGELOG.md`](../../CHANGELOG.md) entry in the same change, then running the two release checks: `node scripts/check-payload-links.mjs` for links that fail in a target, and `node scripts/check-openspec-config.mjs` for a config the OpenSpec CLI cannot read. The [payload rule](../../.claude/rules/payload.md) owns the release steps.
...
- [Scheduled routines](../../.claude/skills/routine/SKILL.md) — `/routine` puts any prompt or verb on a Paseo schedule, each run in its own worktree.
