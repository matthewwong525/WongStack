...
- **If the instruction is a pasted review block** — it begins `Review notes from review.html` — the reviewer walked [the change's review page](../plan/references/review-kit.html) and annotated it. Each numbered line carries where the note sits (`#/<visual>/<state>`, or the What Changes list) and which element it is on, so you can find every one. **Fold them into the change before working any task:** invoke the `openspec-update-change` skill (via the Skill tool) with the notes, so the proposal, design, specs, and tasks stay coherent with each other, and append one Decision-log line naming which note changed what. A note that asks for something already true, or that you decline, is answered in that line rather than silently dropped. Then continue with the tasks.
