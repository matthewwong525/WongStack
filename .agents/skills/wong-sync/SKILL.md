---
name: wong-sync
description: Bring this repo up to date with WongStack. Refreshes a cached upstream clone, follows upstream's own latest instructions, then analyses instead of overwriting: the gap becomes ONE OpenSpec change — an after-picture of what you gain and lose — that you review and /apply. Writes no payload file (except a fresh install), never modifies a file with local authorship, never opens a PR; verdicts land in .claude/wong-sync-verdicts.md, where a ticked box overrules the next run. Use to sync/update/upgrade WongStack here, pull the latest skills, or change what a previous sync decided.
user-invocable: true
---
