...
// Resolve playwright from the APP's node_modules, never from this script's own
...
// somewhere else would run a version the repo never chose.
const require = createRequire(path.join(APP_DIR, 'package.json'))
let chromium
try {
  ;({ chromium } = require('playwright'))
} catch (err) {
  console.error(`walk-runner: cannot load playwright from ${APP_DIR}: ${err.message}`)
  process.exit(2)
}
...
// One probe before any journey runs. Finding the wall here costs one page load
// and produces an honest UNKNOWN; finding it inside a journey produces N
// screenshots of a login form that a grader has to be relied upon to notice.
{
  const probeContext = await browser.newContext({ extraHTTPHeaders: accessHeaders })
...
for (const file of files) {
  ...
  // wrong selector spends it in full, inside a merge gate on a budget. 15s is
  // long enough for a cold Worker and short enough that a handful of bad
  // guesses still leaves time to walk the rest.
  ...
  try {
    ...
    // One broken journey must not cost the evidence for every later one.
