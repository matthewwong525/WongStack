...
[openspec/changes/<name>/review.html](<repo URL>/blob/<branch>/openspec/changes/<name>/review.html) — this change, one picture at a time: click a What Changes entry, see what it changes, annotate it, and press Copy notes to get a `/continue` command to paste back. Open the file from a clone (it works on a phone); a forge shows it as source.   ← omit this whole section if the change has no review.html
...
When `/save` resolved an archived handoff, the source is `openspec/changes/archive/YYYY-MM-DD-<name>/` and the footer links that archive without offering `/continue`; the active change no longer exists. The review link follows the change into that archive path — it moved with the folder. `/ship` never writes or preserves a separate body — its delegated ordinary `/save` call regenerates this one whether the PR is new or existing.
