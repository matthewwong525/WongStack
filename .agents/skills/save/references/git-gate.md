...
[openspec/changes/<name>/wireframe.html](<repo URL>/blob/<branch>/openspec/changes/<name>/wireframe.html) — a clickable mock-up of the screens. Open the file from a clone; a forge shows it as source.   ← omit this whole section if the change has no wireframe.html
...
When `/save` resolved an archived handoff, the source is `openspec/changes/archive/YYYY-MM-DD-<name>/` and the footer links that archive without offering `/continue`; the active change no longer exists. The wireframe link follows the change into that archive path — it moved with the folder. `/ship` never writes or preserves a separate body — its delegated ordinary `/save` call regenerates this one whether the PR is new or existing.
