...
An argument that is a description rather than an existing change name is implementation intent for a new plan. Never let an unrelated sole `openspec list` entry override work the current conversation has just explored. If several candidates remain and the intent does not resolve one, ask the user — [the candidates are the options](../explore/references/asking-the-user.md), each with what it would implement; do not guess. A different branch and change name is valid.
