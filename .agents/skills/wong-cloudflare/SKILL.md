...
1. **Enumerate** what a run creates — the two databases, both Workers, and any Access resources — and show the list. Include any **service token `/walk` minted for itself** ([the walkthrough's self-repair](../../../wiki/development/staging-walkthrough.md#when-the-walk-cant-get-in) creates one named for the repo when it meets the login wall), so a credential this pack caused to exist is not left behind by the pack that removes it.
