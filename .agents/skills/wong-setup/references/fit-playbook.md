...
| "Shipping is manual and scary" / "I forget steps" | `/save` + `/ship` | `/save` owns commit, push, PR, and CI (including `/ship`'s delegated final checkpoint); `/ship` archives, delegates that checkpoint, then squash-merges on green — or on PR review where there's no CI |
