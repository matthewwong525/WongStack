...
5. **Existing skills** — `.claude/skills/`: anything that collides with the payload names (`explore`, `plan`, `apply`, `save`, `continue`, `ship`, `verify`, `wong-sync`, `agent-browser`).
...
- **OpenSpec ready** — the planning layer the skills front: the CLI available (global install, or an `npx @fission-ai/openspec` fallback where global installs are blocked), then `openspec init` run in the target with `--tools` matching the agent(s) from Step 2/Step 6 (already init'd → leave it, just confirm the CLI).
  ...
  - **Correct the CLI's parting advice.** `openspec init` finishes by printing *"Getting started: Start your first change: `/opsx:propose`"*. That command is not installed — the CLI generates the six `openspec-*` skills and no `/opsx:*` commands — and it's the last thing on the user's screen. Immediately say the true version: *"One correction to what OpenSpec just printed: here you start a change with `/plan`, not `/opsx:propose` — that command isn't installed."*
  - **Hide the generated skills from the menu.** Right after `openspec init`, run `"$(git rev-parse --show-toplevel)"/.claude/skills/wong-sync/scripts/hide-openspec-skills.sh` and report its `patched N, already-hidden M` line. It adds `user-invocable: false` to each generated skill, so the user's first `/` menu offers the WongStack verbs and not the six raw OpenSpec steps they should never reach for. The verbs still invoke them through the Skill tool. `openspec init` rewrites these files, so this runs after it, never before.
