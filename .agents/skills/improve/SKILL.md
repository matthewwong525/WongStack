---
name: improve
description: Survey any codebase as a senior advisor and produce prioritized, self-contained plans for OTHER agents to execute — OpenSpec change folders when the repo plans with OpenSpec, plans/NNN-*.md otherwise. Strictly read-only — never implements or fixes anything itself. /improve docs audits the repo's progressive-disclosure wiki (broken links, orphans, duplication, stale docs). Use when asked to audit a codebase, find improvement opportunities (bugs, security, performance, tech debt, DX), suggest features or roadmap direction, or generate handoff plans. Adapted from shadcn/improve (MIT).
user-invocable: true
license: MIT
metadata:
  author: shadcn
