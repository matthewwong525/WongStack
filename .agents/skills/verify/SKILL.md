---
name: verify
description: Verify the deployed preview end to end and show what the change actually does — the on-demand evidence verb. Scouts the change's own OpenSpec scenarios first and matches each to the strongest probe available — a browser journey through the UI, a direct HTTP request probe, or a state probe through an existing command — so a change with nothing observable costs nothing; when there are journeys it runs /save (push, wait for CI, resolve the per-commit preview URL), drives them (agent-browser with a real Chrome for UI journeys, plain requests for the rest), captures evidence per step, grades each against the scenario's written THEN, and posts the evidence as a PR comment on every verdict. Scenarios no probe can reach are listed by name as unverified, never silently dropped. Works in any repo on any stack — the browser is a standalone CLI installed on the machine, never a dependency added to your repository, and a walk with no UI journeys needs no browser at all. Mints a Cloudflare Access service token and retries once when it meets a login wall, then reports UNKNOWN honestly. On a failure inside this change's own code it fixes, re-saves, and re-verifies, at most twice. Gates nothing — /ship runs it once for evidence and merges on the CI result regardless, pausing only to ask you what to do about a FAILURE — so run it as often as you like, mid-change or right before shipping. Use when you want to verify the change, walk the app, see it working, check whether it looks right, watch the change in a browser, test an API end to end, screenshot the UI, get evidence onto the PR, or confirm a change does what its scenarios promised. Never writes inside the repo, and never merges.
user-invocable: true
---
