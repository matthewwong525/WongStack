...
- **Run `node scripts/check-payload-links.mjs`** alongside the bump. It resolves every internal link against the file set a *target* receives. This repo cannot see the problem by inspection — every link resolves here — so the script is the only detector. A link that resolves nowhere in a target fails.
