...
- **Git-fronting skills keep their CLI and delivery steps.** `/save`·`/continue`·`/ship` own every git action. Preserve the spec-reconciliation, resume, validation, and archive behavior when editing them. The [CLI contract](../skills/plan/references/openspec-cli.md) owns shared mechanics. The vendored `agent-browser` skill remains upstream content; refresh it from upstream rather than hand-editing it.
